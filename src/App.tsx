  ...
  const processFile = async (file: File): Promise<ProcessedFile | null> => {
    try {
      ...
      if (fileType.startsWith('text/')) {
        ...
        zip.folder('ppt/slides')?.forEach(async (relativePath, file) => {
          const slideContent = await file.async('string');
          const slideText = slideContent.match(/<a:t>(.*?)<\/a:t>/g)?.map((text: string) => text.replace(/<\/?a:t>/g, '')).join(' ') || '';
      ...
      <main className="max-w-7xl mx-auto px-4 py-6 sm:px-6 lg:px-8">
        ...
        <div className="mt-6 bg-white rounded-lg shadow p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700">Line Endings</label>
              <select
